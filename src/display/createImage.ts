...
export function updateImageProperties(document: DisplayDocument, elementId: string, patch: Partial<ImageProperties>): DisplayDocument {
  let changed = false;
  const elements = document.elements.map((element) => element.id === elementId && element.type === IMAGE_TYPE
    ? (changed = true, { ...element, properties: { ...element.properties, ...patch, rotation: normalizeRotation({ ...element.properties, ...patch }.rotation) } } as ImageElement)
    : element);
