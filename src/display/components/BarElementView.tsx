  ...
  return (
    ...
    >
      ...
      {tagLabel && options.showTagName && (
        <text x={barCenterX} y={element.y + (isPiVisionCompactGauge ? 12 : horizontal ? 16 : 22)} textAnchor="middle" fill={borderColor} fontSize={isPiVisionCompactGauge ? Math.max(8, Math.min(12, element.width * 0.16)) : horizontal ? Math.max(16, Math.min(22, element.height * 0.16)) : 18} fontWeight={500} pointerEvents="none">
          {tagLabel}
        </text>
      )}
      {horizontal && (
        <text x={barCenterX} y={element.y + 42} textAnchor="middle" fill={borderColor} fontSize={Math.max(14, Math.min(20, element.height * 0.13))} fontWeight={500} pointerEvents="none">
      ...
      {options.showScale !== false && !horizontal && isValidScale(minimum, maximum) && Array.from({ length: 9 }, (_, index) => {
        ...
        return <g key={`bar-scale-${index}`} pointerEvents="none"><line x1={plotX - 4 - borderClearance / 2} y1={y} x2={plotX - borderClearance / 2} y2={y} stroke={borderColor} /><text x={plotX - (isPiVisionCompactGauge ? 5 : 14) - borderClearance} y={y + (isPiVisionCompactGauge ? 3 : 6)} textAnchor="end" fill={borderColor} fontSize={isPiVisionCompactGauge ? Math.max(6, Math.min(9, element.width * 0.12)) : 18} fontWeight={500}>{formatScaleValue(valueAtTick, options.decimals)}</text></g>;
      })}
      {options.showScale !== false && horizontal && isValidScale(minimum, maximum) && Array.from({ length: 9 }, (_, index) => {
        const valueAtTick = minimum + ((maximum - minimum) * index) / 8;
        const x = plotX + (plotWidth * index) / 8;
        const scaleY = plotY + plotHeight + borderClearance / 2 + 4;
        return <g key={`bar-scale-horizontal-${index}`} pointerEvents="none"><line x1={x} y1={scaleY} x2={x} y2={scaleY + 4} stroke={borderColor} /><text x={x} y={scaleY + 18} textAnchor="middle" fill={borderColor} fontSize={16} fontWeight={500}>{formatScaleValue(valueAtTick, options.decimals)}</text></g>;
      })}
      {ratio !== undefined && (
        <rect
      ...
      <text x={barCenterX} y={element.y + element.height - (isPiVisionCompactGauge ? 3 : 12)} textAnchor="middle" fill={borderColor} fontSize={isPiVisionCompactGauge ? Math.max(7, Math.min(11, element.width * 0.15)) : Math.max(12, Math.min(24, element.height * 0.12))} data-testid={`bar-value-${element.id}`} pointerEvents="none">
