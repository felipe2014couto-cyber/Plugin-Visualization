...
import type { TableDataExportFormat } from '../../tableDataExport';

export interface TablePropertiesPanelProps { properties: TableProperties; onChange: (patch: Partial<TableProperties>) => void; onRemoveItem: (index: number) => void; onMoveItem: (index: number, offset: -1 | 1) => void; onExport?: (format: TableDataExportFormat) => void; exporting?: boolean; }
export function TablePropertiesPanel({ properties, onChange, onRemoveItem, onMoveItem, onExport, exporting = false }: TablePropertiesPanelProps) {
...
function StyleChoice({ label, value, selected, onSelect }: { label: string; value: 'dark' | 'light' | 'striped'; selected: boolean; onSelect: () => void }) {
  ...
  return <button type="button" aria-pressed={selected} title={label} onClick={onSelect} data-testid={`table-style-${value}`} style={{ borderColor: selected ? 'var(--accent)' : 'var(--border-color)' }}><span style={{ background: colors[0] }} /><span style={{ background: colors[1] }} /><span style={{ background: colors[2] }} />{label}</button>;
}
function getStyles() { return { panel: css`flex:0 0 300px;width:300px;min-width:0;min-height:0;max-height:100%;box-sizing:border-box;overflow-x:hidden;overflow-y:auto;scrollbar-gutter:stable;border-left:1px solid var(--border-color);background:var(--panel-bg);color:var(--text-primary);`, header: css`padding:12px 14px;border-bottom:1px solid var(--border-color);font-weight:600;display:flex;justify-content:space-between; span{font-size:11px;color:var(--text-secondary);font-weight:400;}`, section: css`display:flex;flex-direction:column;gap:8px;padding:12px 14px;border-bottom:1px solid var(--border-color);font-size:12px; label{display:flex;flex-direction:column;gap:4px;color:var(--text-secondary);} select,input{min-height:28px;background:var(--input-bg);color:var(--text-primary);border:1px solid var(--border-color);box-sizing:border-box;}`, styleChoices: css`display:flex;gap:8px;button{width:78px;padding:4px;border:2px solid;background:var(--input-bg);color:var(--text-primary);font-size:10px;display:grid;grid-template-rows:9px 9px 9px auto;gap:1px;cursor:pointer;}span{display:block;}`, columnList: css`width:100%;height:222px;padding:4px;border:1px solid var(--border-color);background:var(--input-bg);color:var(--text-primary);option{padding:2px 4px;font-size:12px;}`, columnSettings: css`display:grid;grid-template-columns:1fr 94px;gap:8px;align-items:center;padding-top:2px;label{flex-direction:row;align-items:center;gap:5px;font-size:11px;}select{width:100%;}`, exportHint: css`color:var(--text-secondary);font-size:11px;line-height:1.35;`, exportActions: css`display:flex;gap:8px;button{flex:1;min-height:30px;border:1px solid var(--border-color);background:var(--button-bg);color:var(--text-primary);cursor:pointer;}button:hover:not(:disabled){background:var(--button-hover);border-color:var(--accent);}button:disabled{opacity:.5;cursor:default;}`, item: css`display:flex;align-items:center;gap:4px;padding:2px;border:1px solid transparent;span{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}button{border:1px solid var(--border-color);background:var(--input-bg);color:var(--text-primary);font-size:11px;min-height:24px;}button:disabled{opacity:.45;}`, itemSelected: css`border-color:var(--accent);`, itemName: css`flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:left;`, itemSettings: css`display:flex;flex-direction:column;gap:8px;padding:8px;background:var(--selection-bg);border:1px solid var(--border-color);` }; }
