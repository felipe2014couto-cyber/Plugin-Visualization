...
export function TrendPropertiesPanel({ element, onVisualChange, onSeriesChange, onClose }: {
  ...
  return <aside className={styles.panel} data-testid="trend-properties-panel" aria-label="Opções da Trend">
    ...
    <label>Escala<select value={visual.scaleMode === 'multiple' ? 'single' : visual.scaleMode} onChange={(e) => onVisualChange({ scaleMode: e.currentTarget.value as TrendScaleMode })}><option value="single">Escala única</option><option value="configurable">Escala configurável</option></select></label>
