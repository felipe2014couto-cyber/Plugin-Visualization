...
export function TrendPropertiesPanel({ element, onVisualChange, onSeriesChange, onClose }: {
  ...
  return <aside className={styles.panel} data-testid="trend-properties-panel" aria-label="Opções da Trend">
    ...
    {series.length > 1 && <label className={styles.check}><input type="checkbox" checked={selected.primaryScale === true} onChange={(e) => onSeriesChange(selectedKey, { primaryScale: e.currentTarget.checked })} />★ Escala principal (direita)</label>}
