  ...
  return (
    <aside className={styles.panel} data-testid={`${kind.toLowerCase()}-properties-panel`} aria-label={`Configuração do ${kind}`}>
      ...
      <div className={styles.fields}>
        ...
        {kind === 'Bar' && tagNameMode === 'custom' && <label className={styles.field}><span>Rótulo personalizado</span><input value={customBarNameDraft} onChange={(event) => { setCustomBarNameDraft(event.target.value); onChange({ customTagName: event.target.value }); }} onBlur={() => { if (!customBarNameDraft.trim() && pointName) { setCustomBarNameDraft(pointName); onChange({ customTagName: pointName }); } }} data-testid="bar-custom-tag-name" /></label>}
        {kind === 'Gauge' && <label className={styles.field}><span>Rótulo personalizado</span><input value={gaugeTitleDraft} onChange={(event) => { setGaugeTitleDraft(event.target.value); onChange({ title: event.target.value }); }} onBlur={() => { if (!gaugeTitleDraft.trim() && pointName) { setGaugeTitleDraft(pointName); onChange({ title: pointName }); } }} placeholder="Nome da tag" data-testid="gauge-title" /></label>}
