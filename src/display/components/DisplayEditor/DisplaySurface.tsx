...
function calculationValueRuntimeState(
  ...
  const nextState: ValueRuntimeState = { status: 'success', result: { value: evaluation.value, timestamp: maxTimestamp || new Date().toISOString() } };
      ...
      {(() => {
        const renderSingleElement = (element: DisplayElement): React.ReactNode => {
          ...
          if (element.type === TEXT_TYPE) {
            ...
            const textColor = getMultistateColor(runtimeVal, textElement.properties.multistate, resolveThemeForeground(textElement.properties.color), textSourceValue);
            ...
            const rawText = decodeRenderedText(textElement.properties.text || '');
            const lines = rawText.includes('\n') ? rawText.split('\n') : [rawText];
            const fontSize = textElement.properties.fontSize || 16;
            const lineHeight = fontSize * 1.25;
            const totalHeight = lines.length * lineHeight;
            const startY = textElement.y + (textElement.height - totalHeight) / 2 + fontSize * 0.85;
            return (
              <g
                key={element.id}
                x={textElement.x}
              ...
              >
                ...
                >
                  {lines.length === 1 ? (
                    <tspan x={x} y={textElement.y + textElement.height / 2} dominantBaseline="middle">
                      {lines[0]}
                    </tspan>
                  ) : (
                    lines.map((line, idx) => (
                      <tspan key={idx} x={x} y={startY + idx * lineHeight}>
                        {line}
                      </tspan>
                    ))
                  )}
...
function renderGeometricShape(element: RectangleElement, runtimeState?: ValueRuntimeState, parentElementId?: string, sourceBindingValues?: Map<string, unknown>) {
  ...
  const fill = getMultistateColor(value, element.properties.multistate, baseFill, sourceValue);
