import React, { useState } from 'react';
import { SqlTableElement } from '../createSqlTable';
import { SqlResultTable, getDynamicStyles } from '../../components/SqlQuery/SqlResultTable';
import { SqlFullscreenPopup } from './SqlFullscreenPopup';

interface SqlTableElementViewProps {
  element: SqlTableElement;
  selected?: boolean;
  editable?: boolean;
}

export function SqlTableElementView({ element, selected, editable }: SqlTableElementViewProps) {
  const styleObj = getDynamicStyles(element.properties);
  const [showPopup, setShowPopup] = useState(false);

  const handleDoubleClick = (e: React.MouseEvent) => {
    // Open full-screen popup ONLY when in view mode (not editable)
    if (editable) {
      return;
    }
    e.stopPropagation();
    e.preventDefault();
    setShowPopup(true);
  };

  const handleFullscreenClick = (e: React.MouseEvent) => {
    if (editable) {
      return;
    }
    e.stopPropagation();
    e.preventDefault();
    setShowPopup(true);
  };

  return (
    <g
      data-testid={`display-element-${element.id}`}
      data-element-id={element.id}
      data-element-type={element.type}
      style={{ cursor: editable ? 'move' : 'default' }}
      transform={`translate(${element.x}, ${element.y})`}
    >
      <foreignObject
        x={0}
        y={0}
        width={element.width}
        height={element.height}
        onDoubleClick={!editable ? handleDoubleClick : undefined}
      >
        <div
          onDoubleClick={!editable ? handleDoubleClick : undefined}
          style={{
            width: '100%',
            height: '100%',
            boxSizing: 'border-box',
            border: selected ? '2px solid var(--focus-ring)' : '1px solid var(--accent)',
            borderRadius: 10,
            overflow: 'hidden',
            background: 'var(--sql-row-bg, var(--surface-primary))',
            boxShadow: 'var(--shadow)',
            display: 'flex',
            flexDirection: 'column',
            position: 'relative',
            ...styleObj,
          }}
        >
          {(element.properties.showTitle ?? true) && (
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
                padding: '8px 12px',
                background: element.properties.titleTransparent
                  ? 'transparent'
                  : 'var(--sql-header-bg, var(--surface-elevated))',
                borderBottom: element.properties.titleTransparent
                  ? 'none'
                  : '1px solid var(--sql-border-color, var(--border-color))',
                color: 'var(--sql-text-color, var(--text-primary))',
                fontSize: `${element.properties.titleFontSize ?? 20}px`,
                fontWeight: 500,
                textAlign: element.properties.titleAlign ?? 'left',
                cursor: editable ? 'move' : 'default',
              }}
            >
              <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {element.properties.title || 'Resultado da consulta SIP'}
              </span>
              {!editable && (
                <button
                  onClick={handleFullscreenClick}
                  title="Abrir em tela cheia"
                  style={{
                    background: 'transparent',
                    border: 'none',
                    color: 'var(--sql-text-color, #94a3b8)',
                    cursor: 'pointer',
                    fontSize: 16,
                    padding: '2px 6px',
                    borderRadius: 4,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    transition: 'background 0.2s, color 0.2s',
                    pointerEvents: 'all',
                  }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.1)';
                    e.currentTarget.style.color = '#ffffff';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = 'transparent';
                    e.currentTarget.style.color = 'var(--sql-text-color, #94a3b8)';
                  }}
                >
                  ⛶
                </button>
              )}
            </div>
          )}
          <div
            style={{ flex: 1, minHeight: 0, overflow: 'hidden', cursor: 'default' }}
            onDoubleClick={!editable ? handleDoubleClick : undefined}
          >
            <SqlResultTable
              result={element.properties.result ?? null}
              isLoading={false}
              properties={element.properties}
            />
          </div>
        </div>
      </foreignObject>

      {editable && (
        <rect
          x={0}
          y={0}
          width={element.width}
          height={element.height}
          fill="transparent"
          pointerEvents="all"
          data-element-id={element.id}
          data-element-type={element.type}
        />
      )}

      {!editable && showPopup && (
        <SqlFullscreenPopup
          element={element}
          onClose={() => setShowPopup(false)}
        />
      )}
    </g>
  );
}
