...
export interface XYPlotYSeries { binding: PiPointBinding; pairing?: XYPairing; timestampMatch?: XYTimestampMatch; color?: string; marker?: XYMarkerStyle; connectLine?: boolean; regression?: boolean; correlation?: boolean; recentCount?: number; recentColor?: string; scaleMode?: XYScaleMode; min?: number; max?: number; }
...
export function getXYPlotYSeries(properties: XYPlotProperties): XYPlotYSeries[] {
  ...
  return source.filter((item): item is XYPlotYSeries => isPiPointBinding(item?.binding) && bindingKey(item.binding) !== bindingKey(properties.xBinding) && !used.has(bindingKey(item.binding)) && Boolean(used.add(bindingKey(item.binding)))).map((item) => ({ ...item, binding: { ...item.binding }, pairing: item.pairing ?? 'timestamp', timestampMatch: item.timestampMatch ?? 'interpolated', marker: item.marker ?? 'circle', color: item.color ?? '#6e9fff', connectLine: item.connectLine ?? properties.connectPoints ?? false, scaleMode: item.scaleMode ?? 'plotted' }));
}
export function normalizeXYPlotProperties(properties: XYPlotProperties): XYPlotProperties { const ySeries = getXYPlotYSeries(properties); return { ...properties, ySeries, ...(ySeries[0] ? { yBinding: { ...ySeries[0].binding } } : {}), connectPoints: ySeries.some((item) => item.connectLine), multipleYScales: properties.multipleYScales === true, xScaleMode: properties.xScaleMode ?? 'plotted', showTitle: properties.showTitle !== false, showLegend: properties.showLegend !== false, showGrid: properties.showGrid !== false, format: properties.format ?? 'general' }; }
export function createXYPlot(options: { xBinding: PiPointBinding; yBinding?: PiPointBinding; ySeries?: XYPlotYSeries[]; id?: string; x?: number; y?: number; width?: number; height?: number; surface?: DisplaySurface; existingIds?: readonly string[]; generateId?: () => string }): XYPlotElement {
  ...
  return { id, type: XY_PLOT_TYPE, x: options.x ?? Math.max(0, ((surface?.width ?? width) - width) / 2), y: options.y ?? Math.max(0, ((surface?.height ?? height) - height) / 2), width, height, properties: normalizeXYPlotProperties({ xBinding: { ...options.xBinding }, ...(options.yBinding ? { yBinding: { ...options.yBinding } } : {}), ...(options.ySeries ? { ySeries: options.ySeries } : {}) }) };
}
...
export function addXYPlotYSeries(document: DisplayDocument, elementId: string, binding: PiPointBinding): DisplayDocument { return updateElementInDocument(document, elementId, (element) => { if (element.type !== XY_PLOT_TYPE) return element; const properties = normalizeXYPlotProperties((element as XYPlotElement).properties); if (bindingKey(binding) === bindingKey(properties.xBinding) || getXYPlotYSeries(properties).some((item) => bindingKey(item.binding) === bindingKey(binding))) return element; return { ...element, properties: normalizeXYPlotProperties({ ...properties, ySeries: [...getXYPlotYSeries(properties), { binding: { ...binding } }] }) } as XYPlotElement; }); }
