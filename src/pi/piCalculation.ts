...
function normalizeValue(item: Record<string, unknown>): PiCalculationResult {
  ...
  if (nested && typeof nested === 'object' && !Array.isArray(nested)) {
    ...
    if (Object.prototype.hasOwnProperty.call(nestedRecord, 'Value')
      ...
      return normalizeValue({ ...item, ...nestedRecord });
    }
  }
  ...
  if (good === false) throw new PiCalculationExecutionError('PI Calculation Controller retornou Good=false.');
...
export async function evaluatePiExpression(request: PiCalculationRequest): Promise<PiCalculationResult> {
  ...
  const key = [request.binding.dataSourceUid, webId, request.expression, mode, request.time ?? '', request.startTime ?? '', request.endTime ?? '', request.interval ?? '', request.summaryType ?? ''].join('\u0000');
  const cached = evaluationCache.get(key);
  if (cached && cached.expiresAt > Date.now()) return cached.value;
  const inflight = evaluationLock.get(key);
  if (inflight) return inflight;
  const evaluation = getPiResource<unknown>(request.binding.dataSourceUid, queryFor(mode, webId, request.expression, request), request.dataSourceSrv)
    .then((response) => normalizeValue(valuesFromResponse(response)[0]))
    .catch((error) => {
