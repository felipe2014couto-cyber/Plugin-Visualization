...
rsync -avz --exclude='venv' --exclude='.venv' --exclude='__pycache__' "$PROJECT_DIR/backend-python/" "$REMOTE_USER@$REMOTE_HOST:/tmp/pims-vision-deploy/backend-python/"
...
ssh "$REMOTE_USER@$REMOTE_HOST" "sudo bash" << 'EOF'
  set -euo pipefail
  echo "Parando grafana-server para atualizacao de plugins..."
  ...
  if [ -d /usr/share/grafana/public ]; then
    install -o root -g root -m 0644 /tmp/pims-vision-dashboard-redirect.js /usr/share/grafana/public/pims-vision-dashboard-redirect.js 2>/dev/null || install -o root -g root -m 0644 /tmp/pims-vision-deploy/pims-vision-dashboard-redirect.js /usr/share/grafana/public/pims-vision-dashboard-redirect.js
